...
from losses.free_anchor import BoxCoder, FreeAnchorLoss
...
class RetinaHead(nn.Module):
    ...
    def forward(self, xs):
        ...
        for j, x in enumerate(xs):
            cls_outputs.append(self.cls_head(self.cls_bones(x)))
            reg_outputs.append(self.scales[j](self.reg_head(self.reg_bones(x))))
...
class RetinaNet(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        if self.training:
            ...
            positive_loss, negative_loss = self.loss(cls_outputs, reg_outputs, anchors, targets)
            ret['positive_loss'] = positive_loss
            ret['negative_loss'] = negative_loss
        else:
            _, _, h, w = x.shape
            for pred in out:
