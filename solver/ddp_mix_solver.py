...
class DDPMixSolver(object):
    def __init__(self, cfg_path):
        ...
        self.pos_loss_logger = AverageLogger()
        self.neg_loss_logger = AverageLogger()

    def train(self, epoch):
        self.loss_logger.reset()
        self.pos_loss_logger.reset()
        self.neg_loss_logger.reset()
        self.model.train()
        ...
        for i, (img_tensor, targets_tensor, batch_len) in enumerate(pbar):
            ...
            if self.scaler is not None:
                with amp.autocast(enabled=True):
                    ...
                    positive_loss = out['positive_loss']
                    negative_loss = out['negative_loss']
            ...
            self.pos_loss_logger.update(positive_loss.item())
            self.neg_loss_logger.update(negative_loss.item())
            str_template = "epoch:{:2d}|size:{:3d}|loss:{:6.4f}|pos:{:6.4f}|neg:{:6.4f}|lr:{:8.6f}"
            if self.local_rank == 0:
                pbar.set_description(str_template.format(
                    epoch,
                    h,
                    self.loss_logger.avg(),
                    self.pos_loss_logger.avg(),
                    self.neg_loss_logger.avg(),
                    lr)
                )
        self.ema.update_attr(self.model)
        loss_avg = reduce_sum(torch.tensor(self.loss_logger.avg(), device=self.device)) / self.gpu_num
        pos_loss_avg = reduce_sum(torch.tensor(self.pos_loss_logger.avg(), device=self.device)).item() / self.gpu_num
        neg_loss_avg = reduce_sum(torch.tensor(self.neg_loss_logger.avg(), device=self.device)).item() / self.gpu_num
        if self.local_rank == 0:
            final_template = "epoch:{:2d}|loss:{:6.4f}|pos:{:6.4f}|neg:{:6.4f}"
